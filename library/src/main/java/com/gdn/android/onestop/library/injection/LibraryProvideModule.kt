...
class LibraryProvideModule {
  ...
  fun provideLibraryDatabase(application: Application): LibraryDatabase {
    return Room.databaseBuilder(application.applicationContext, LibraryDatabase::class.java,
        "library_database").fallbackToDestructiveMigration().build()
  }
  ...
  @LibraryScope
  @Provides
  fun provideBookRepository(context: Context, libraryClient: LibraryClient,
      libraryDao: LibraryDao, bookUpdateManager: BookUpdateManager): BookRepository {
    return BookRepository(context, libraryDao, libraryClient, bookUpdateManager)
  }

  @LibraryScope
  @Provides
  fun provideBookUpdateManager(application: Application,
      sessionManager: SessionManager): BookUpdateManager {
